	...
	max_label("max")
{
	...
	for(  int np = 0/*, count = 1*/;  np < MAX_PLAYER_COUNT;  np++  ) {
		...
		// [mod : shingoushori] Add MAP_LINES mode to show not has_convoi players.
		else if(  welt->get_player( np )  &&  welt->get_player( np )->has_money_or_assets()) {
			viewed_player_c.append_element( new gui_scrolled_list_t::const_text_scrollitem_t(welt->get_player( np )->get_name(), 0xFFFF));
			viewable_players[ count++ ] = np;
		}
	}
	for(  int np = 0;  np < MAX_PLAYER_COUNT;  np++  ) {
		if(  welt->get_player( np )  &&  welt->get_player( np )->get_finance()->has_convoi()) {
			...
			viewed_player_c.append_element( new gui_scrolled_list_t::const_text_scrollitem_t(welt->get_player( np )->get_name(), 0x0000));
			viewable_players[ count++ ] = np | 16;
		}
	}
