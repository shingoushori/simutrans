...
void convoi_info_t::draw(scr_coord pos, scr_size size)
{
	if(!cnv.is_bound()) {
		...
		// [mod : shingoushori] To enable sharing convoi lines owned by public player, among players.  2/4 - 1
		if((cnv->get_owner()==welt->get_active_player()||welt->get_public_player()==welt->get_active_player())  &&  !welt->get_active_player()->is_locked()) {
...
bool convoi_info_t::action_triggered( gui_action_creator_t *comp,value_t /* */)
{
	...
	// [mod : shingoushori] To enable sharing convoi lines owned by public player, among players.  2/4 - 2
	if((cnv->get_owner()==welt->get_active_player()||welt->get_public_player()==welt->get_active_player())  &&  !welt->get_active_player()->is_locked()) {
	//if(cnv->get_owner()==welt->get_active_player()  &&  !welt->get_active_player()->is_locked()) {
